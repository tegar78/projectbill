...
<div class="d-sm-flex align-items-center justify-content-between mb-3">
    <?php if ($this->session->userdata('role_id') == 1 or $role['add_bill'] == 1) { ?>
        <a href="" id="#addModal" data-toggle="modal" data-target="#addModal" class="nm-btn nm-btn-primary"><i class="fas fa-plus mr-1"></i> Tambah Tagihan</a>
    <?php } ?>
</div>
    ...
    <div class="col-12 col-lg-12">
        <div class="nm-card mb-4" id="tablebill-card">
            <div class="d-flex align-items-center justify-content-between mb-3 pb-2" style="border-bottom: 1px solid var(--nm-shadow-dark);">
                <h5 class="m-0 font-weight-bold" style="color: var(--nm-text-main);"><i class="fas fa-file-invoice mr-2" style="color: var(--nm-brand);"></i> Data Tagihan Belum Bayar <span id="area"></span></h5>
            </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" style="width: 100%;" name="year" required>
                                ...
                                <?php if ($this->session->userdata('role_id') == 1) {  ?>
                                    ...
                                    <?php } ?>
                                <?php } ?>
                                <?php if (date('m') == 12 or $this->session->userdata('role_id') != 1) {  ?>
                                    <?php
                                    for ($i = date('Y') + 1; $i >= date('Y') - 2; $i -= 1) {
                                    ?>
                                        <option value="<?= $i ?>"><?= $i ?></option>
    ...
    <script>
        $(document).ready(function() {
            $('#example').DataTable({
                ...
                "order": [],
                "ajax": {
                    "url": "<?= base_url('bill/getUnpaidInvoice'); ?>",
                    "type": "POST"
                },
                "lengthMenu": [
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            type: 'get',
            url: '<?= site_url('whatsapp/sendduedate') ?>',
            cache: false,
            success: function(data) {}
        });
        $.ajax({
            type: 'get',
            url: '<?= site_url('whatsapp/sendbeforeduedate') ?>',
            cache: false,
            success: function(data) {}
        });
        $.ajax({
            type: 'get',
            url: '<?= site_url('whatsapp/resendbill') ?>',
            cache: false,
            success: function(data) {}
        });
