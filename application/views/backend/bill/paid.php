...
$cur_m = date('m');
$cur_y = date('Y');
$has_cur_m = $this->db->get_where('invoice', ['month' => $cur_m, 'year' => $cur_y])->num_rows();
if ($has_cur_m > 0) {
    $def_month = $cur_m;
    $def_year = $cur_y;
} else {
    $latest_inv = $this->db->select('month, year')->order_by('year', 'DESC')->order_by('month', 'DESC')->get('invoice', 1)->row();
    if ($latest_inv) {
        $def_month = sprintf('%02d', (int)$latest_inv->month);
        $def_year = $latest_inv->year;
    } else {
        $def_month = $cur_m;
        $def_year = $cur_y;
    }
...
<?php if ($totalcustomer >= 500) { ?>
    <?php if ($this->session->userdata('role_id') == 1 or $menu['coverage_menu'] == 1) { ?>
        <div class="nm-card mb-3">
            <div class="d-flex align-items-center mb-2">
                <span class="nm-label" style="color: var(--nm-brand);"><i class="fas fa-filter mr-1"></i> Filter Tagihan</span>
            </div>
            <div class="row">
                <div class="col-lg-4 col-sm-12 col-md-6">
                    <div class="form-group mb-2">
                        <label class="nm-label mb-2" for="">Coverage Area</label>
                        <select class="form-control select2 nm-input" style="width: 100%;" name="coverage" id="coverage" onchange="getdatabill()" required>
                ...
                <div class="col-lg-4 col-sm-12 col-md-6">
                    <div class="form-group mb-2">
                        <label class="nm-label mb-2" for="">Bulan</label>
                        <select class="form-control select2 nm-input" style="width: 100%;" name="month" id="month" onchange="getdatabill()">
                            <option value="all" <?= $def_month == 'all' ? 'selected' : '' ?>>Semua Bulan</option>
                            <option value="01" <?= $def_month == '01' ? 'selected' : '' ?>>Januari</option>
                            <option value="02" <?= $def_month == '02' ? 'selected' : '' ?>>Februari</option>
                            <option value="03" <?= $def_month == '03' ? 'selected' : '' ?>>Maret</option>
                            <option value="04" <?= $def_month == '04' ? 'selected' : '' ?>>April</option>
                            <option value="05" <?= $def_month == '05' ? 'selected' : '' ?>>Mei</option>
                            <option value="06" <?= $def_month == '06' ? 'selected' : '' ?>>Juni</option>
                            <option value="07" <?= $def_month == '07' ? 'selected' : '' ?>>Juli</option>
                            <option value="08" <?= $def_month == '08' ? 'selected' : '' ?>>Agustus</option>
                            <option value="09" <?= $def_month == '09' ? 'selected' : '' ?>>September</option>
                            <option value="10" <?= $def_month == '10' ? 'selected' : '' ?>>Oktober</option>
                            <option value="11" <?= $def_month == '11' ? 'selected' : '' ?>>November</option>
                            <option value="12" <?= $def_month == '12' ? 'selected' : '' ?>>Desember</option>
                        </select>
                    </div>
                </div>
                <div class="col-lg-4 col-sm-12 col-md-6">
                    <div class="form-group mb-2">
                        <label class="nm-label mb-2" for="">Tahun</label>
                        <select class="form-control select2 nm-input" style="width: 100%;" name="year" id="year" onchange="getdatabill()">
                            <option value="all" <?= $def_year == 'all' ? 'selected' : '' ?>>Semua Tahun</option>
                            <?php
                            $start_y = (int)date('Y') + 1;
                            $end_y = (int)date('Y') - 3;
                            for ($i = $start_y; $i >= $end_y; $i -= 1) { ?>
                                <option value="<?= $i ?>" <?= $def_year == $i ? 'selected' : '' ?>><?= $i ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                <input type="hidden" id="status" value="SUDAH BAYAR">
            </div>
        </div>
    <?php } ?>
<?php } ?>
...
<div class="row">
    <div class="col-12 col-lg-12">
        <div class="nm-card mb-4" id="tablebill-card">
            <div class="d-flex align-items-center justify-content-between mb-3 pb-2" style="border-bottom: 1px solid var(--nm-shadow-dark);">
                <h5 class="m-0 font-weight-bold" style="color: var(--nm-text-main);"><i class="fas fa-check-circle mr-2 text-success"></i> Data Tagihan Sudah Bayar <span id="area"></span></h5>
            </div>
                ...
                <div class="row">
                    <div class="col-6">
                        <div class="form-group">
                            ...
                            <input type="" name="invoice" value="<?= $invoice ?>">
                            <select class="form-control select2" style="width: 100%;" name="month" required>
                                <option value="<?= date('m') ?>"><?= indo_month(date('m')) ?></option>
                                <option value="01">Januari</option>
                                <option value="02">Februari</option>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row text-center">
                    <div class="col">
                        <a id="printpaida4" href="<?= site_url('bill/printinvoicepaid') ?>" target="blank" class="btn btn-outline-success" style="font-size: smaller"><i class="fa fa-print"> A4</i></a>
                    </div>
                    <div class="col">
                        <a id="printpaidthermal" href="<?= site_url('bill/printinvoicepaidthermal') ?>" target="blank" class="btn btn-outline-success" style="font-size: smaller"><i class="fa fa-print"> Thermal</i></a>
                    </div>
                    <div class="col">
                        <a id="printpaiddot" href="<?= site_url('bill/printinvoicepaidthermal') ?>" target="blank" class="btn btn-outline-success" style="font-size: smaller"><i class="fa fa-print"> Dot Matrix</i></a>
                    </div>
                    <div class="col">
                        <a id="printpaidsmall" href="<?= site_url('bill/printinvoicesmall') ?>" target="blank" class="btn btn-outline-success" style="font-size: smaller"><i class="fa fa-print"> Small</i></a>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="row text-center">
                    <div class="col">
                        <a href="<?= site_url('bill/printinvoiceunpaid') ?>" class="btn btn-outline-danger" style="font-size: smaller"><i class="fa fa-print"> A4</i></a>
                    </div>
                    <div class="col">
                        <a href="<?= site_url('bill/printinvoiceunpaidthermal') ?>" class="btn btn-outline-danger" style="font-size: smaller"><i class="fa fa-print"> Thermal</i></a>
                    </div>
                    <div class="col">
                        <a href="<?= site_url('bill/printunpaid') ?>" class="btn btn-outline-danger" style="font-size: smaller"><i class="fa fa-print"> Print Table</i></a>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            $('#example').DataTable({
                ...
                "order": [],
                "ajax": {
                    "url": "<?= base_url('bill/getPaidInvoice'); ?>",
                    "type": "POST"
                },
                "lengthMenu": [
...
<script>
    $(document).ready(function() {
        <?php if (isset($totalcustomer) && $totalcustomer >= 500) { ?>
            getdatabill();
        <?php } ?>
    });
