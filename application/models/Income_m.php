...
{
    public $order = array(null, null, 'date_payment', 'nominal', 'category', 'mode_payment', 'remark', null);
    ...
    public function getTotalIncome()
    {
        ...
        $query = $this->db->get('income')->row_array();
        return isset($query['nominal']) ? (float)$query['nominal'] : 0;
    }
    ...
    public function getReport($date = null, $month = null, $year = null)
    {
        ...
        $cat_income = $this->db->get('cat_income')->result_array();
        $cat_income_map = array_column($cat_income, 'name', 'category_id');

        $cat_expenditure = $this->db->get('cat_expenditure')->result_array();
        $cat_expenditure_map = array_column($cat_expenditure, 'name', 'category_id');

        $rows = [];
        foreach ($income as $data) {
            $category_name = isset($cat_income_map[$data->category]) ? $cat_income_map[$data->category] : '';
            $rows[] = array(
                'date' => $data->date_payment,
                'income' => (float)$data->nominal,
                'expenditure' => 0,
                'remark' => $data->remark,
        ...
        foreach ($expenditure as $data) {
            $category_name = isset($cat_expenditure_map[$data->category]) ? $cat_expenditure_map[$data->category] : '';
            $rows2[] = array(
                'date' => $data->date_payment,
                'income' => 0,
                'expenditure' => (float)$data->nominal,
                'remark' => $data->remark,
                'category' => $category_name
            );
        }
