...
class Bill_m extends CI_Model
{
    ...
    public $column_order = array(null, null, null,  'invoice', 'invoice.no_services', 'customer.name', 'customer.no_wa', 'inv_due_date', 'amount',  'status', 'address');
    public $order =  array(null, null, 'customer.name', 'customer.no_wa', 'customer.no_services', 'invoice.invoice',  'month',  'amount', 'status', 'coverage', null);
    public $orderfilter =  array(null, null, 'customer.name', 'customer.no_wa', 'customer.no_services', 'invoice.invoice',  'month', 'inv_due_date',  'amount', 'status', 'coverage', null);
    public $orderunpaid =  array(null, null, 'customer.name', 'customer.no_wa', 'customer.no_services', 'invoice.invoice',  'month', 'inv_due_date',  'amount', 'status', 'coverage', null);
    public $orderpaid =  array(null, null, 'customer.name', 'customer.no_wa', 'customer.no_services', 'invoice.invoice',  'month', 'inv_due_date',  'amount', 'status', 'coverage', null);
    public $orderduedate =  array(null, null, 'customer.name', 'invoice.no_services',  'inv_due_date', 'amount');
    ...
    public function getInvoiceduedate($row = null)
    {
        ...
        $this->db->where('status', 'BELUM BAYAR');
        $this->db->where('invoice.inv_due_date >=', '2018-01-01');
        $this->db->where('invoice.inv_due_date <=', $datetoday);
        $query = $this->db->get();
        return $query;
    }
    ...
    public function getInvoiceWhatsapp($status, $month, $year)
    {
        ...
        $this->db->group_start();
        $this->db->where('send_bill', '');
        $this->db->or_where('send_bill IS NULL', null, false);
        $this->db->group_end();
        $this->db->order_by('created_invoice', 'DESC');
        $query = $this->db->get();
        return $query;
    ...
    private function _get_data_query()
    {
        ...
        if (isset($_POST['order']) && isset($_POST['order']['0']['column'])) {
            ...
            if (isset($this->order[$col_idx]) && $this->order[$col_idx] !== null) {
                $this->db->order_by($this->order[$col_idx], $col_dir);
            } else {
                $this->db->order_by('invoice', 'desc');
            }
        } else {
    ...
    private function _get_data_queryunpaid()
    {
        ...
        $role = $this->db->get_where('role_management', ['role_id' => $role_id])->row_array();
        $row = [];
        if (isset($role['coverage_operator']) && $role['coverage_operator'] == 1) {
            $operator = $this->db->get_where('cover_operator', ['operator' => $this->session->userdata('id')])->result();
            ...
            if (isset($_POST['search']['value']) && $_POST['search']['value'] !== '') {
                ...
                $this->db->or_like('invoice.invoice', $_POST['search']['value']);
                $this->db->or_like('customer.name', $_POST['search']['value']);
                $this->db->or_like('customer.no_wa', $_POST['search']['value']);
                $this->db->group_end();
            }
        } else {
            $this->db->select('*, invoice.created as created_invoice');
        ...
        if (isset($_POST['order']) && isset($_POST['order']['0']['column'])) {
            $col_idx = (int)$_POST['order']['0']['column'];
            $col_dir = $_POST['order']['0']['dir'];
            if (isset($this->orderunpaid[$col_idx]) && $this->orderunpaid[$col_idx] !== null) {
                $this->db->order_by($this->orderunpaid[$col_idx], $col_dir);
            } else {
                $this->db->order_by('invoice', 'desc');
            }
        } else {
    ...
    private function _get_data_querypaid()
    {
        ...
        if (isset($_POST['order']) && isset($_POST['order']['0']['column'])) {
            $col_idx = (int)$_POST['order']['0']['column'];
            $col_dir = $_POST['order']['0']['dir'];
            if (isset($this->orderpaid[$col_idx]) && $this->orderpaid[$col_idx] !== null) {
                $this->db->order_by($this->orderpaid[$col_idx], $col_dir);
            } else {
                $this->db->order_by('invoice', 'desc');
            }
        } else {
    ...
    public function getDataInv()
    {
        ...
        if (isset($_POST['length']) && $_POST['length'] != -1) {
            $this->db->limit((int)$_POST['length'], (int)(isset($_POST['start']) ? $_POST['start'] : 0));
        }
        $query = $this->db->get();
        return $query->result();
    }
    ...
    private function _get_data_queryduedate()
    {
        ...
        $role = $this->db->get_where('role_management', ['role_id' => $role_id])->row_array();
        $row = [];
        if ($this->session->userdata('role_id') != 1 && isset($role['coverage_operator']) && $role['coverage_operator'] == 1) {
            $operator = $this->db->get_where('cover_operator', ['operator' => $this->session->userdata('id')])->result();
        ...
        if (isset($_POST['order']) && isset($_POST['order']['0']['column'])) {
            $col_idx = (int)$_POST['order']['0']['column'];
            $col_dir = $_POST['order']['0']['dir'];
            if (isset($this->orderduedate[$col_idx]) && $this->orderduedate[$col_idx] !== null) {
                $this->db->order_by($this->orderduedate[$col_idx], $col_dir);
            } else {
    ...
    private function _get_data_queryfiltercoverage($post)
    {
        ...
        if (isset($role['coverage_operator']) && $role['coverage_operator'] == 1) {
            ...
            if (isset($post['coverage'])) {
                if ($post['coverage'] == '') {
                    $this->db->where('customer.coverage', 0);
                } elseif ($post['coverage'] == 'all') {
                    $this->db->where_in('customer.coverage', $row);
                } else {
                    $this->db->where('customer.coverage', $post['coverage']);
                }
            }
        } else {
            if (isset($post['status'])) {
        ...
        if (isset($post['month']) && $post['month'] != '' && $post['month'] != 'all') {
            $m = (int)$post['month'];
            $m_str = sprintf('%02d', $m);
            $this->db->where_in('invoice.month', [$m, $m_str, (string)$m]);
        }
        if (isset($post['year']) && $post['year'] != '' && $post['year'] != 'all') {
            $this->db->where('invoice.year', $post['year']);
        }
        ...
        if (isset($_POST['order']) && isset($_POST['order']['0']['column'])) {
            $col_idx = (int)$_POST['order']['0']['column'];
            $col_dir = $_POST['order']['0']['dir'];
            if (isset($this->orderfilter[$col_idx]) && $this->orderfilter[$col_idx] !== null) {
                $this->db->order_by($this->orderfilter[$col_idx], $col_dir);
            } else {
                $this->db->order_by('invoice.invoice', 'desc');
            }
        } else {
            $this->db->order_by('invoice.invoice', 'desc');
        }
