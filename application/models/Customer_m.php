...
class Customer_m extends CI_Model
{
    ...
    var $order = array(null, null, 'name', 'no_services',   'email', 'no_wa', 'due_date', 'ppn', 'cust_amount', 'address', null);
    ...
    public function getfilterby($post, $row = null)
    {
        $this->db->select('*');
        $this->db->from('customer');
        if (isset($post['status']) && $post['status'] != null) {
            $this->db->where('c_status', $post['status']);
        }
        if (isset($post['router']) && $post['router'] != 0) {
    ...
    public function count_all_data_active($cover = null)
    {
        $this->db->from($this->table);
        $this->db->where('c_status', 'Aktif');
        if ($cover != null) {
    ...
    public function getmaps()
    {
        $this->db->select('*');
        $this->db->from('customer');
        $this->db->where("latitude IS NOT NULL AND latitude != '' AND latitude != '0'", NULL, FALSE);
        $this->db->where("longitude IS NOT NULL AND longitude != '' AND longitude != '0'", NULL, FALSE);
        $this->db->order_by('name', 'ASC');
        $query = $this->db->get();
        return $query;
    }
    public function getmaps_detail()
    {
        ...
        $this->db->join('m_odp', 'm_odp.id_odp = customer.id_odp', 'left');
        $this->db->where("customer.latitude IS NOT NULL AND customer.latitude != '' AND customer.latitude != '0'", NULL, FALSE);
        $this->db->where("customer.longitude IS NOT NULL AND customer.longitude != '' AND customer.longitude != '0'", NULL, FALSE);
