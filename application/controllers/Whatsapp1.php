    ...
    public function sendbill()
    {
        ...
        if ($whatsapp['is_active'] == 1) {
            ...
            foreach ($datainvoice as $wa) {
                ...
                // Optimistic marking — tandai sebelum kirim agar tidak dikirim ulang
                $this->db->set('send_bill', date('Y-m-d H:i:s'));
                $this->db->where('invoice', $wa->invoice);
                ...
                if ($whatsapp['is_active'] == 1) {
                    if ($whatsapp['duedateinvoice'] == 1) {
                        ...
                        foreach ($bill as $data) {
                            ...
                            // Optimistic marking — tandai sebelum kirim agar cron berikutnya tidak kirim ulang
                            $this->db->set('send_due', date('Y-m-d H:i:s'));
                ...
                if ($whatsapp['is_active'] == 1) {
                    if ($whatsapp['duedateinvoice'] == 1) {
                        ...
                        foreach ($bill as $data) {
                            ...
                            // Optimistic marking — tandai sebelum kirim agar cron berikutnya tidak kirim ulang
                            $this->db->set('send_before_due', date('Y-m-d H:i:s'));
                            $this->db->where('invoice', $data->invoice);
                            $this->db->update('invoice');
