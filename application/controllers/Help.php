...
class Help extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($role_id != 1 && isset($role['show_help']) && $role['show_help'] == 0) {
        ...
        $cektiket = $this->help_m->getcekticket(isset($customer['no_services']) ? $customer['no_services'] : '')->row_array();
    ...
    public function del()
    {
        $role_id = $this->session->userdata('role_id');
        $role = $this->db->get_where('role_management', ['role_id' => $role_id])->row_array();
        if ($role_id != 1 && isset($role['del_help']) && $role['del_help'] == 0) {
            ...
            if (empty($help)) {
                ...
                if (file_exists($target_file) && !empty($help['picture'])) {
                    unlink($target_file);
                }
                $this->db->where('id', $id);
                $this->db->delete('help');
    ...
    public function addhelp()
    {
        if ($this->input->method() !== 'post') {
            redirect($this->session->userdata('role_id') == 2 ? 'help/history' : 'help/data');
            return;
        }

        $post = $this->input->post(null, TRUE);
        if (empty($post['no_services']) || empty($post['type'])) {
            $this->session->set_flashdata('error-sweet', 'Data tiket tidak lengkap');
            $referer = $this->input->server('HTTP_REFERER');
            redirect(!empty($referer) && strpos($referer, 'addhelp') === false ? $referer : 'help/data');
            return;
        }
        ...
        $solution = !empty($post['solution']) ? $this->db->get_where('help_solution', ['hs_id' => $post['solution']])->row_array() : null;
        ...
        $redirect_user = function() {
            if ($this->session->userdata('role_id') == 2) {
                ...
                $referer = $this->input->server('HTTP_REFERER');
                if (!empty($referer) && strpos($referer, 'addhelp') === false) {
                    redirect($referer);
                } else {
                    redirect('help/data');
                }
                ...
                if ($this->db->affected_rows() > 0) {
                    ...
                    if (isset($whatsapp['is_active']) && $whatsapp['is_active'] == 1) {
                        ...
                        if ($whatsapp['create_help'] == 1 or $whatsapp['create_help_admin'] == 1) {
                            ...
                            $maps = !empty($maps_info['route_url']) ? $maps_info['route_url'] : 'Belum disetting';
                            $search  = array('{name}', '{noservices}', '{phone}', '{address}', '{email}', '{noticket}', '{topic}', '{report}', '{remark}', '{maps}', '{companyname}', '{slogan}', '{link}', '{e}');
                            $replace = array($customer['name'], $customer['no_services'], indo_tlp($customer['no_wa']), $customer['address'], $customer['email'], isset($cekhelp['no_ticket']) ? $cekhelp['no_ticket'] : '', isset($type['help_type']) ? $type['help_type'] : '', isset($solution['hs_name']) ? $solution['hs_name'] : '', isset($post['remark']) ? $post['remark'] : '', $maps, isset($company['company_name']) ? $company['company_name'] : '', isset($company['sub_name']) ? $company['sub_name'] : '', base_url(), '');
                            $subject = isset($other['create_help']) ? $other['create_help'] : '';
                            ...
                            if ($whatsapp['create_help'] == 1) {
                                ...
                                foreach ($teknisi as $data) {
                                    $range = $no++ * (isset($whatsapp['interval_message']) ? $whatsapp['interval_message'] : 1);
                    ...
                    if (!empty($bot['token']) && !empty($bot['id_group_teknisi'])) {
                        ...
                        $tg_type = isset($type['help_type']) ? $type['help_type'] : '';
                        $tg_solution = isset($solution['hs_name']) ? $solution['hs_name'] : '';
                        $tg_remark = isset($post['remark']) ? $post['remark'] : '';
                        $sendmessage = [
                            'reply_markup' => json_encode(['inline_keyboard' => []]),
                            'resize_keyboard' => true,
                            'parse_mode' => 'html',
                            'text' => "<b>TIKET BARU {$tg_type}</b>\nNama : {$customer['name']}\nEmail : {$customer['email']}\nNo WA : {$customer['no_wa']}\nAlamat : {$customer['address']}\nTopik Gangguan : {$tg_type}\nLaporan : {$tg_solution}\nKeterangan : {$tg_remark}\nStatus : Pending\nCreate By : $createby ($level)",
                            'chat_id' => $idgroupteknisi
                        ];
                        $ctx = stream_context_create(['http' => ['timeout' => 3]]);
                        @file_get_contents("https://api.telegram.org/bot$tokens/sendMessage?" . http_build_query($sendmessage), false, $ctx);
                    }
                }
                $redirect_user();
            } else {
    ...
    public function getsolutiondetail()
    {
        ...
        echo isset($solution['solution']) ? $solution['solution'] : '';
    }
    ...
    public function history()
    {
        ...
        $customer = $this->db->get_where('customer', ['email' => $this->session->userdata('email')])->row_array();
        $data['help'] = $this->help_m->history(isset($customer['no_services']) ? $customer['no_services'] : '')->result();
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['customer'] = $this->db->get_where('customer', ['email' => $this->session->userdata('email')])->row_array();
        $data['company'] = $this->db->get('company')->row_array();
    ...
    public function pending()
    {
        ...
        $data['customertiket'] = $this->customer_m->getCustomertiket()->result();
        $data['help'] = $this->help_m->getpending()->result();
        $this->template->load('backend', 'backend/help/data', $data);
    }
    ...
    public function gettiket()
    {
        ...
        if (isset($cek['teknisi']) && $cek['teknisi'] != 0) {
            $user = $this->db->get_where('user', ['id' => $cek['teknisi']])->row_array();
            $this->session->set_flashdata('error-sweet', 'Data tiket sedang ditangani oleh ' . (isset($user['name']) ? $user['name'] : 'Teknisi') . '');
            redirect($_SERVER['HTTP_REFERER']);
        } else {
    ...
    public function donecustomer()
    {
        $post = $this->input->post(null, TRUE);
        $cek = $this->db->get_where('help', ['id' => $post['id']])->row_array();
        if (isset($cek['status']) && $cek['status'] == 'close') {
            $this->session->set_flashdata('success-sweet', 'Tiket berhasil diperbaharui');
        } else {
            $this->db->set('status', 'close');
            if (isset($post['status']) && $post['status'] == 'done' && $this->session->userdata('role_id') != 2) {
                $this->db->set('teknisi', $this->session->userdata('id'));
            }
            $this->db->where('id', $post['id']);
            $this->db->update('help');
            ...
            $params = [
                ...
                'date_update' => time(),
                'status' => 'close',
                'remark' => isset($post['description']) ? $post['description'] : '',
                'teknisi' => $this->session->userdata('id'),
            ];
            $this->db->insert('help_timeline', $params);
            if ($this->db->affected_rows() > 0) {
    ...
    public function serverhelp()
    {
        ...
        foreach ($result as $resultRow) {
            ...
            $action = ' 
               <a href="' . site_url('help/detail/' . $resultRow->id) . '" title="Detail"><i class="fa fa-clone" style="font-size:20px; color:green"></i></a>
             
              <a href="" data-toggle="modal" id="deletehelp" data-idhelp="' . $resultRow->id . '" data-tiket="' . $resultRow->no_ticket . '" data-target="#delete" title="Hapus"><i class="fa fa-trash" style="font-size:20px; color:red"></i></a>';

            $type = $this->db->get_where('help_type', ['help_id' => $resultRow->help_type])->row_array();
            $hs = $this->db->get_where('help_solution', ['hs_id' => $resultRow->help_solution])->row_array();

            $row = [
                'no' => ++$no,
                'date_created' => date('d M Y H:i:s', (int)$resultRow->date_created),
                'no_ticket' => htmlspecialchars($resultRow->no_ticket, ENT_QUOTES, 'UTF-8'),
                'name' => htmlspecialchars($resultRow->name, ENT_QUOTES, 'UTF-8'),
                'no_services' => htmlspecialchars($resultRow->no_services, ENT_QUOTES, 'UTF-8'),
                'report' => (isset($type['help_type']) ? $type['help_type'] : '') . '<br>' . (isset($hs['hs_name']) ? $hs['hs_name'] : ''),
                'status' =>  ucwords(strtolower($resultRow->status)),
                'action' => $action,
                'description' => htmlspecialchars($resultRow->description, ENT_QUOTES, 'UTF-8'),
            ];
            $data[] = $row;
        }
        $output = array(
