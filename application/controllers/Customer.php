...
class customer extends CI_Controller
{
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data['company'] = $this->db->get('company')->row_array();
            $data['package'] = $data['company'];
            $data['customer'] = (object)['coverage' => 0];
            if ($this->session->userdata('role_id') == 3 && isset($role['coverage_operator']) && $role['coverage_operator'] == 1) {
    ...
    {
        set_time_limit(120);
        $results = $this->customer_m->getDataTable();
        $company = $this->db->get('company')->row_array();
        $data = [];
        $no = $_POST['start'];

        // Pre-fetch router sekali saja (selalu id=1)
        $rt = $this->db->get_where('router', ['id' => 1])->row_array();

        // Kumpulkan semua no_services dan email untuk batch query
        $all_no_services = [];
        $all_emails_fallback = [];
        $all_emails = [];
        foreach ($results as $r) {
            if ($r->no_services != 0) {
                $all_no_services[] = $r->no_services;
            } else {
                $all_emails_fallback[] = $r->email;
            }
            $all_emails[] = $r->email;
        }

        // Batch query services berdasarkan no_services
        $services_by_ns = [];
        if (!empty($all_no_services)) {
            $this->db->where_in('no_services', $all_no_services);
            $svc_result = $this->db->get('services')->result();
            foreach ($svc_result as $svc) {
                if (!isset($services_by_ns[$svc->no_services])) {
                    $services_by_ns[$svc->no_services] = 0;
                }
                $services_by_ns[$svc->no_services] += $svc->total;
            }
        }

        // Batch query services berdasarkan email (untuk customer dengan no_services == 0)
        $services_by_email = [];
        if (!empty($all_emails_fallback)) {
            $this->db->where_in('email', $all_emails_fallback);
            $svc_result_email = $this->db->get('services')->result();
            foreach ($svc_result_email as $svc) {
                if (!isset($services_by_email[$svc->email])) {
                    $services_by_email[$svc->email] = 0;
                }
                $services_by_email[$svc->email] += $svc->total;
            }
        }

        // Batch query user berdasarkan email
        $users_map = [];
        if (!empty($all_emails)) {
            $unique_emails = array_unique($all_emails);
            $this->db->where_in('email', $unique_emails);
            $user_result = $this->db->get('user')->result();
            foreach ($user_result as $u) {
                $users_map[$u->email] = true;
            }
        }

        foreach ($results as $r) {
            // Ambil subtotal dari data yang sudah di-prefetch
            if ($r->no_services != 0) {
                $subtotal = isset($services_by_ns[$r->no_services]) ? $services_by_ns[$r->no_services] : 0;
            } else {
                $subtotal = isset($services_by_email[$r->email]) ? $services_by_email[$r->email] : 0;
            }

            $row = array();
            $row[] = ++$no;
            $row[] = '<input type=' . 'checkbox' . ' class=' . 'check-item' . ' id="ceklis" name=' . 'customer_id[]' . ' value=' . $r->customer_id . '>';
            $row[] = $r->name;
            $row[] = $r->no_services;

            // Gunakan data user yang sudah di-prefetch
            if (isset($users_map[$r->email])) {
                $row[] = $r->email . '<br>Akses Login <i class="fa fa-check" aria-hidden="true" style="color:green"></i>';
            } else {
                $row[] = $r->email . '<br>Akses Login <i class="fa fa-times" aria-hidden="true" style="color:red"></i>';
            }
            if ($r->phonecode == 0) {
                $phone = $company['phonecode'] . '' . $r->no_wa;
            } else {
                $phone =  $r->phonecode . '' . $r->no_wa;
            }
            $row[] = indo_tlp($r->no_wa);
            $row[] = $r->c_status;
            ...
            $row[] = $r->address;
            // Gunakan data router yang sudah di-prefetch
            if ($rt['is_active'] == 1) {
                $row[] =
                    '<a href="' . site_url('services/detail/') . $r->no_services . '"    title="Detail Paket" style="font-size: smaller"><i class="fa fa-eye" style="font-size:25px; color:gray"></i></a> 
                    <a href="' . site_url('customer/print/') . $r->no_services . '"    title="Cetak" target="blank" style="font-size: smaller"><i class="fa fa-print" style="font-size:25px; color:brown"></i></a> 
<a href="' . site_url('customer/edit/') . $r->customer_id . '" title="Edit"><i class="fa fa-edit" style="font-size:25px"></i></a> 
<a href="' . site_url('mikrotik/client/') . $r->no_services . '" title="Koneksi"><i class="fa fa-random" style="font-size:25px;color:green"></i></a>                     
                <a href="#" id="delete" data-customer_id="' . $r->customer_id . '" data-no_services="' . $r->no_services . ' " data-name=' . $r->name . ' data-toggle="modal" data-target="#DeleteModal"   title="Delete" ><i class="fa fa-trash" style="font-size:25px; color:red"></i></a> 
                ';
            } else {
                $row[] = '<a href="' . site_url('services/detail/') . $r->no_services . '"    title="Detail Paket" style="font-size: smaller"><i class="fa fa-eye" style="font-size:25px; color:gray"></i></a> 
                <a href="' . site_url('customer/edit/') . $r->customer_id . '" title="Edit"><i class="fa fa-edit" style="font-size:25px"></i></a> 
                <a href="#" id="delete" data-customer_id="' . $r->customer_id . '" data-no_services="' . $r->no_services . ' " data-name=' . $r->name . ' data-toggle="modal" data-target="#DeleteModal"   title="Delete" ><i class="fa fa-trash" style="font-size:25px; color:red"></i></a> 
                ';
            }
        ...
        $this->output->set_content_type('application/json')->set_output(json_encode($output, JSON_INVALID_UTF8_SUBSTITUTE));
    }
    public function getActiveCustomer()
    {
        ...
        try {
            file_put_contents(APPPATH . 'logs/debug.log', date('Y-m-d H:i:s') . " - Start getActiveCustomer\n", FILE_APPEND);
            $results = $this->customer_m->getActive();
            file_put_contents(APPPATH . 'logs/debug.log', date('Y-m-d H:i:s') . " - Got " . count($results) . " results\n", FILE_APPEND);
            $data = [];
            $company = $this->db->get('company')->row_array();
            $no = $_POST['start'];
            ...
            // Batch query services berdasarkan no_services (chunked)
            $services_by_ns = [];
            if (!empty($all_no_services)) {
                $ns_chunks = array_chunk(array_unique($all_no_services), 200);
                foreach ($ns_chunks as $chunk) {
                    $this->db->where_in('no_services', $chunk);
                    $svc_result = $this->db->get('services')->result();
                    foreach ($svc_result as $svc) {
                        if (!isset($services_by_ns[$svc->no_services])) {
                            $services_by_ns[$svc->no_services] = 0;
            ...
            // Batch query services berdasarkan email (chunked)
            $services_by_email = [];
            if (!empty($all_emails_fallback)) {
                $email_chunks = array_chunk(array_unique($all_emails_fallback), 200);
            ...
            // Batch query user berdasarkan email (chunked)
            $users_map = [];
            if (!empty($all_emails)) {
                $unique_emails = array_unique($all_emails);
                $u_chunks = array_chunk($unique_emails, 200);
                foreach ($u_chunks as $chunk) {
                    $this->db->where_in('email', $chunk);
                    $user_result = $this->db->get('user')->result();
                    foreach ($user_result as $u) {
                        $users_map[$u->email] = true;
            ...
            foreach ($results as $r) {
                ...
                $row[] = indo_tlp($r->no_wa);
                $row[] = $r->c_status . '<br>Jatuh Tempo : ' . $r->due_date;
...
<a href="' . site_url('mikrotik/client/') . $r->no_services . '" title="Koneksi"><i class="fa fa-random" style="font-size:25px;color:green"></i></a>                     
            ...
            if ($role['role_id'] != 1 && $role['coverage_operator'] == 1) {
                $coverage = $this->customer_m->count_all_data_active($cover);
            } else {
                $coverage = $this->customer_m->count_all_data_active();
            }
            ...
            file_put_contents(APPPATH . 'logs/debug.log', date('Y-m-d H:i:s') . " - Output ready. Records filtered: " . $output['recordsFiltered'] . "\n", FILE_APPEND);

            $this->output->set_content_type('application/json')->set_output(json_encode($output, JSON_INVALID_UTF8_SUBSTITUTE));
            file_put_contents(APPPATH . 'logs/debug.log', date('Y-m-d H:i:s') . " - Output sent\n", FILE_APPEND);
        } catch (Throwable $e) {
            file_put_contents(APPPATH . 'logs/customer_ajax_error.log', $e->getMessage() . "\n" . $e->getTraceAsString());
            $output = array(
    ...
    public function getNonActiveCustomer()
    {
        ...
        foreach ($temp_results as $tmp) {
            if ($tmp->no_services != 0) {
                $all_no_services[] = $tmp->no_services;
            }
            $all_emails[] = $tmp->email;
        }
...
foreach ($result as $result) {
            $subtotal = isset($services_by_ns[$result->no_services]) ? $services_by_ns[$result->no_services] : 0;
            $row = array();
            $row[] = ++$no;
            $row[] = '<input type=' . 'checkbox' . ' class=' . 'check-item' . ' id="ceklis" name=' . 'customer_id[]' . ' value=' . $result->customer_id . '>';
            $row[] = $result->name;
            $row[] = $result->no_services;
            $user = isset($users_map[$result->email]) ? 1 : 0;
...
foreach ($result as $result) {
    ...
    {
        $post = $this->input->post(null, TRUE);
        $coverage = isset($post['coverage']) ? (int)$post['coverage'] : 0;
        
        $this->db->select('*');
        $this->db->from('services');
        $this->db->join('customer', 'customer.no_services = services.no_services');
        $this->db->where('customer.c_status', 'Aktif');
        if ($coverage > 0) {
            $this->db->where('customer.coverage', $coverage);
        }
        $querying = $this->db->get()->result();
        $grandtotal = 0;
        foreach ($querying as $dataa) {
            $grandtotal += (int) $dataa->total;
        }
