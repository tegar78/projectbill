...
class Mikrotik extends CI_Controller
{
    ...
    private function _coverage()
    {
        $role = $this->_role();
        $row = [];
        if (isset($role['role_id']) && $role['role_id'] != 1 && isset($role['coverage_operator']) && $role['coverage_operator'] == 1) {
    ...
    public function editsetting()
    {
        ...
        $API = new Mikweb();
        $API->connect($ip, $user, $pass, $port);
        $getisolir = is_array($API->comm("/system/schedule/print", array('?name' => 'ISOLIR-MY-WIFI',))) ? $API->comm("/system/schedule/print", array('?name' => 'ISOLIR-MY-WIFI',)) : [];
        $idisolir = isset($getisolir['0']['.id']) ? $getisolir['0']['.id'] : null;

        $getresetpppoe = is_array($API->comm("/system/schedule/print", array('?name' => 'RESET-PPPOE',))) ? $API->comm("/system/schedule/print", array('?name' => 'RESET-PPPOE',)) : [];
        $idresetpppoe = isset($getresetpppoe['0']['.id']) ? $getresetpppoe['0']['.id'] : null;

        $getresethotspot = is_array($API->comm("/system/schedule/print", array('?name' => 'RESET-HOTSPOT',))) ? $API->comm("/system/schedule/print", array('?name' => 'RESET-HOTSPOT',)) : [];
        $idresethotspot = isset($getresethotspot['0']['.id']) ? $getresethotspot['0']['.id'] : null;

        $getresetstatic = is_array($API->comm("/system/schedule/print", array('?name' => 'RESET-STATIC',))) ? $API->comm("/system/schedule/print", array('?name' => 'RESET-STATIC',)) : [];
        $idresetstatic = isset($getresetstatic['0']['.id']) ? $getresetstatic['0']['.id'] : null;

        $getcountpppoe = is_array($API->comm("/system/schedule/print", array('?name' => 'COUNT-PPPOE',))) ? $API->comm("/system/schedule/print", array('?name' => 'COUNT-PPPOE',)) : [];
        $idcountpppoe = isset($getcountpppoe['0']['.id']) ? $getcountpppoe['0']['.id'] : null;

        $getcountstandalone = is_array($API->comm("/system/schedule/print", array('?name' => 'COUNT-STANDALONE',))) ? $API->comm("/system/schedule/print", array('?name' => 'COUNT-STANDALONE',)) : [];
        $idcountstandalone = isset($getcountstandalone['0']['.id']) ? $getcountstandalone['0']['.id'] : null;

        $getcreatebill = is_array($API->comm("/system/schedule/print", array('?name' => 'GENERATE-BILL',))) ? $API->comm("/system/schedule/print", array('?name' => 'GENERATE-BILL',)) : [];
        $idcreatebill = isset($getcreatebill['0']['.id']) ? $getcreatebill['0']['.id'] : null;

        $getreminderduedate = is_array($API->comm("/system/schedule/print", array('?name' => 'REMINDER-BILL',))) ? $API->comm("/system/schedule/print", array('?name' => 'REMINDER-BILL',)) : [];
        $idreminderduedate = isset($getreminderduedate['0']['.id']) ? $getreminderduedate['0']['.id'] : null;
        $getreminderduedatebefore = is_array($API->comm("/system/schedule/print", array('?name' => 'REMINDER-BEFORE-DUE',))) ? $API->comm("/system/schedule/print", array('?name' => 'REMINDER-BEFORE-DUE',)) : [];
        $idreminderduedatebefore = isset($getreminderduedatebefore['0']['.id']) ? $getreminderduedatebefore['0']['.id'] : null;

        $getreminderduedatetelegram = is_array($API->comm("/system/schedule/print", array('?name' => 'REMINDER-BILL-TELEGRAM',))) ? $API->comm("/system/schedule/print", array('?name' => 'REMINDER-BILL-TELEGRAM',)) : [];
        $idreminderduedatetelegram = isset($getreminderduedatetelegram['0']['.id']) ? $getreminderduedatetelegram['0']['.id'] : null;

        $getbackupdb = is_array($API->comm("/system/schedule/print", array('?name' => 'BACKUP-DB',))) ? $API->comm("/system/schedule/print", array('?name' => 'BACKUP-DB',)) : [];
        $idbackupdb = isset($getbackupdb['0']['.id']) ? $getbackupdb['0']['.id'] : null;

        $jamreminderduedatetelegram = isset($post['jamreminderduedatetelegram']) && strlen($post['jamreminderduedatetelegram']) == 1 ? '0' . $post['jamreminderduedatetelegram'] : (isset($post['jamreminderduedatetelegram']) ? $post['jamreminderduedatetelegram'] : '00');
        $menitreminderduedatetelegram = isset($post['menitreminderduedatetelegram']) && strlen($post['menitreminderduedatetelegram']) == 1 ? '0' . $post['menitreminderduedatetelegram'] : (isset($post['menitreminderduedatetelegram']) ? $post['menitreminderduedatetelegram'] : '00');

        $jambackupdb = isset($post['jambackupdb']) && strlen($post['jambackupdb']) == 1 ? '0' . $post['jambackupdb'] : (isset($post['jambackupdb']) ? $post['jambackupdb'] : '00');
        $menitbackupdb = isset($post['menitbackupdb']) && strlen($post['menitbackupdb']) == 1 ? '0' . $post['menitbackupdb'] : (isset($post['menitbackupdb']) ? $post['menitbackupdb'] : '00');
    ...
    public function getdatacustomer()
    {
        ...
        foreach ($result as $resultRow) {
            if (isset($resultRow->auto_isolir) && $resultRow->auto_isolir == 1) {
                $isolir = 'Aktif';
            } else {
                $isolir = 'Tidak Aktif';
            }
            if (isset($resultRow->action) && $resultRow->action == 1) {
                $user_profile = isset($resultRow->user_profile) ? $resultRow->user_profile : '';
                $action = 'Pindah Profile <br> Profile Default : ' . $user_profile;
            } else {
                $action = 'Disable User';
            }
            $row = array();
            $row[] = ++$no;
            $row[] = htmlspecialchars($resultRow->no_services, ENT_QUOTES, 'UTF-8') . '<br><a href="' . site_url('mikrotik/client/' . $resultRow->no_services) . '" class="badge badge-success">Detail</a>';
            $row[] = htmlspecialchars($resultRow->name, ENT_QUOTES, 'UTF-8');
            $row[] = isset($resultRow->alias) ? htmlspecialchars($resultRow->alias, ENT_QUOTES, 'UTF-8') : '';
            $row[] = isset($resultRow->mode_user) ? htmlspecialchars($resultRow->mode_user, ENT_QUOTES, 'UTF-8') : '';
            $row[] = isset($resultRow->user_mikrotik) ? htmlspecialchars($resultRow->user_mikrotik, ENT_QUOTES, 'UTF-8') : '';

            $usageList = $this->mikrotik_m->usagethismonth($resultRow->no_services)->result();
            $totalusage = 0;
            foreach ($usageList as $uRow) {
                $totalusage += isset($uRow->count_usage) ? $uRow->count_usage : 0;
            }

            $last = $this->mikrotik_m->lastusage($resultRow->no_services)->row_array();
            if (!empty($last) && isset($last['last_update'])) {
                $lastupdate = '<br>Last Update ' . date('d M Y - H:i:s', $last['last_update']);
            } else {
                $lastupdate = '';
            }
            if ($totalusage > 0) {
                $row[] = formatBites($totalusage, 2) . ' <a href="' . site_url('mikrotik/usage/' . $resultRow->no_services) . '" title="Edit"><i class="fa fa-eye"></i></a> ' . $lastupdate;
            } else {
                $row[] = '';
            }
            $row[] = isset($resultRow->due_date) ? $resultRow->due_date : '';
            $row[] = $isolir . '<br> ' . $action;
            $row[] = '<a href="' . site_url('customer/edit/' . $resultRow->customer_id) . '" title="Edit"><i class="fa fa-edit" style="font-size:25px"></i></a>';
            $data[] = $row;
        }
        $output = array(
