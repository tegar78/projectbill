...
class Income extends CI_Controller
{
    ...
    public function kas()
    {
        ...
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['company'] = $this->db->get('company')->row_array();
        $data['report'] = $this->income_m->getReport(null, date('m'), date('Y'));
        $data['category'] = $this->db->get('cat_income')->result();
    ...
    public function getDataIncome()
    {
        ...
        foreach ($result as $resultRow) {
            $row = array();
            $row[] = ++$no;
            $row[] = '<input type="checkbox" class="check-item" id="ceklis" name="income_id[]" value="' . htmlspecialchars($resultRow->income_id, ENT_QUOTES, 'UTF-8') . '">';
            $row[] = indo_date($resultRow->date_payment);
            $getcategory = $this->db->get_where('cat_income', ['category_id' => $resultRow->category])->row_array();
            $row[] = indo_currency($resultRow->nominal);
            if (!empty($resultRow->category) && !empty($getcategory)) {
                $row[] = htmlspecialchars($getcategory['name'], ENT_QUOTES, 'UTF-8');
            } else {
                $row[] = '';
            }
            $row[] = htmlspecialchars($resultRow->mode_payment, ENT_QUOTES, 'UTF-8');
            $row[] = htmlspecialchars($resultRow->remark, ENT_QUOTES, 'UTF-8');
            $row[] = '<a href="#" id="edit"   data-income_id="' . htmlspecialchars($resultRow->income_id, ENT_QUOTES, 'UTF-8') . '" data-nominal="' . $resultRow->nominal . '" data-remark="' . htmlspecialchars($resultRow->remark, ENT_QUOTES, 'UTF-8') . '" data-category="' . $resultRow->category . '" data-mode_payment="' . htmlspecialchars($resultRow->mode_payment, ENT_QUOTES, 'UTF-8') . '" data-receipt="' . $resultRow->create_by . '" data-date_payment="' . $resultRow->date_payment . '" title="Edit" data-toggle="modal" data-target="#Modaledit"><i class="fa fa-edit" style="font-size:25px"></i></a>
            <a  href="#" id="delete" data-toggle="modal" data-income_id="' . htmlspecialchars($resultRow->income_id, ENT_QUOTES, 'UTF-8') . '" data-nominal="' . indo_currency($resultRow->nominal) . '" data-remark="' . htmlspecialchars($resultRow->remark, ENT_QUOTES, 'UTF-8') . '" data-date_payment="' . $resultRow->date_payment . '" data-target="#Modaldelete"  title="Delete"><i class="fa fa-trash" style="font-size:25px; color:red"></i></a>';
            $data[] = $row;
        }
