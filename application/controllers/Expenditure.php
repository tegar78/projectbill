...
class expenditure extends CI_Controller
{
    ...
    public function getDataExpend()
    {
        ...
        foreach ($result as $resultRow) {
            $row = array();
            $row[] = ++$no;
            $row[] = '<input type="checkbox" class="check-item" id="ceklis" name="expenditure_id[]" value="' . htmlspecialchars($resultRow->expenditure_id, ENT_QUOTES, 'UTF-8') . '">';
            $row[] = indo_date($resultRow->date_payment);
            $row[] = indo_currency($resultRow->nominal);
            $getcategory = $this->db->get_where('cat_expenditure', ['category_id' => $resultRow->category])->row_array();
            ...
            $row[] = '<a href="#" id="edit"  data-expenditure_id="' . htmlspecialchars($resultRow->expenditure_id, ENT_QUOTES, 'UTF-8') . '" data-nominal="' . $resultRow->nominal . '" data-remark="' . htmlspecialchars($resultRow->remark, ENT_QUOTES, 'UTF-8') . '" data-date_payment="' . $resultRow->date_payment . '" data-category="' . $resultRow->category . '" title="Edit" data-toggle="modal" data-target="#Modaledit"><i class="fa fa-edit" style="font-size:25px"></i></a>
            <a  href="#" id="delete" data-toggle="modal" data-target="#Modaldelete" data-expenditure_id="' . htmlspecialchars($resultRow->expenditure_id, ENT_QUOTES, 'UTF-8') . '" data-nominal="' . indo_currency($resultRow->nominal) . '" data-remark="' . htmlspecialchars($resultRow->remark, ENT_QUOTES, 'UTF-8') . '" data-date_payment="' . $resultRow->date_payment . '" title="Delete"><i class="fa fa-trash" style="font-size:25px; color:red"></i></a>';
            $data[] = $row;
        }
        $output = array(
